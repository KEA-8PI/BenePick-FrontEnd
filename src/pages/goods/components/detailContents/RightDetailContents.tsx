...
const RightDetailContents = ({ info, like, handleLike }) => {
  return (
    <div>
      ...
      <C.CardBoldFont style={{ fontSize: '20px', textDecoration: 'line-through', color: colors.grey01 }}>
        {info.price.toLocaleString()}원
      </C.CardBoldFont>
      <S.Row>
        <C.CardBoldFont style={{ fontSize: '20px' }}>{info.discountPrice.toLocaleString()}원</C.CardBoldFont>
        <Typography style={{ color: 'black', fontSize: '15px', paddingLeft: '150px' }}>{info.amounts}</Typography>
      </S.Row>
